...
local function RUN (p, ...)
  p = string.gsub(p, "flua", '"'..progname..'"', 1)
...
-- test option '-'
RUN('echo "print(arg[1])" | flua - -h > %s', out)
checkout("-h\n")
...
-- test LUA_PATH
RUN('env LUA_INIT= LUA_PATH=x flua %s > %s', prog, out)
checkout("x\n")

-- test LUA_PATH_version
RUN('env LUA_INIT= LUA_PATH_5_4=y LUA_PATH=x flua %s > %s', prog, out)
checkout("y\n")

-- test LUA_CPATH
prepfile("print(package.cpath)")
RUN('env LUA_INIT= LUA_CPATH=xuxu flua %s > %s', prog, out)
checkout("xuxu\n")

-- test LUA_CPATH_version
RUN('env LUA_INIT= LUA_CPATH_5_4=yacc LUA_CPATH=x flua %s > %s', prog, out)
checkout("yacc\n")

-- test LUA_INIT (and its access to 'arg' table)
prepfile("print(X)")
RUN('env LUA_INIT="X=tonumber(arg[1])" flua %s 3.2 > %s', prog, out)
checkout("3.2\n")

-- test LUA_INIT_version
prepfile("print(X)")
RUN('env LUA_INIT_5_4="X=10" LUA_INIT="X=3" flua %s > %s', prog, out)
checkout("10\n")

-- test LUA_INIT for files
prepfile("x = x or 10; print(x); x = x + 1")
RUN('env LUA_INIT="@%s" flua %s > %s', prog, prog, out)
checkout("10\n11\n")

-- test errors in LUA_INIT
NoRun('LUA_INIT:1: msg', 'env LUA_INIT="error(\'msg\')" flua')
...
do
  ...
  RUN('env LUA_INIT="error(10)" LUA_PATH=xxx LUA_CPATH=xxx flua -E %s > %s',
       prog, out)
...
assert(not string.find(defaultpath, "xxx") and
       string.find(defaultpath, "flua") and
       not string.find(defaultCpath, "xxx") and
       string.find(defaultCpath, "flua"))
...
RUN('env LUA_PATH="?;;" flua -l %s -l%s -lstring -l io %s > %s', prog, otherprog, otherprog, out)
checkout("1\n2\n15\n2\n15\n")
...
prepfile(a)
RUN('flua "-e " -- %s a b c', prog)   -- "-e " runs an empty command

-- test 'arg' availability in libraries
prepfile"assert(arg)"
prepfile("assert(arg)", otherprog)
RUN('env LUA_PATH="?;;" flua -l%s - < %s', prog, otherprog)

-- test messing up the 'arg' table
RUN('echo "print(...)" | flua -e "arg[1] = 100" - > %s', out)
checkout("100\n")
NoRun("'arg' is not a table", 'echo "" | flua -e "arg = 1" -')

-- test error in 'print'
RUN('echo 10 | flua -e "print=nil" -i > /dev/null 2> %s', out)
assert(string.find(getoutput(), "error calling 'print'"))

-- test 'debug.debug'
RUN('echo "io.stderr:write(1000)\ncont" | flua -e "require\'debug\'.debug()" 2> %s', out)
...
RUN('echo "io.stderr:write(1); warn[[XXX]]" | flua 2> %s', out)
checkout("1")
...
-- test many arguments
prepfile[[print(({...})[30])]]
RUN('flua %s %s > %s', prog, string.rep(" a", 30), out)
checkout("a\n")

RUN([[flua "-eprint(1)" -ea=3 -e "print(a)" > %s]], out)
checkout("1\n3\n")
...
print(a)
a]]
RUN([[flua -e"_PROMPT='' _PROMPT2=''" -i < %s > %s]], prog, out)
checkprogout("6\n10\n10\n\n")
...
RUN([[flua "-e_PROMPT='%s'" -i < %s > %s]], prompt, prog, out)
local t = getoutput()
...
RUN([[flua -e "%s" -i < %s > %s]], prompt, prog, out)
local t = getoutput()
...
prepfile("error{}")
NoRun("error object is a table value", [[flua %s]], prog)
...
-- close fLua with an open file
prepfile(string.format([[io.output(%q); io.write('alo')]], out))
RUN('flua %s', prog)
checkout('alo')

-- bug in 5.2 beta (extra \0 after version line)
RUN([[flua -v  -e"print'hello'" > %s]], out)
t = getoutput()
...
-- invalid options
NoRun("unrecognized option '-h'", "flua -h")
NoRun("unrecognized option '---'", "flua ---")
NoRun("unrecognized option '-Ex'", "flua -Ex")
NoRun("unrecognized option '-vv'", "flua -vv")
NoRun("unrecognized option '-iv'", "flua -iv")
NoRun("'-e' needs argument", "flua -e")
NoRun("syntax error", "flua -e a")
NoRun("'-l' needs argument", "flua -l")


if T then   -- test library?
  print("testing 'not enough memory' to create a state")
  NoRun("not enough memory", "env MEMLIMIT=100 flua")
